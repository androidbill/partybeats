...
const APP_VERSION = "2026.07.09.03";
...
function PartyMotionCanvas({ className = "" }) {
  ...
  useEffect(() => {
    ...
    // Hard-stop the loop while the app is backgrounded (e.g. a guest is off in
    // YouTube Music copying a link). Browsers throttle rAF in background tabs,
    // but an explicit stop guarantees zero work and avoids a burst of catch-up
    // frames on resume.
    const stopLoop = () => {
...
function App() {
  ...
  useEffect(() => {
    ...
    panel.scrollTo({
      top: Math.max(0, panel.scrollTop + rowRect.top - panelRect.top),
      left: 0,
      behavior: "smooth"
    });
                    ...
                    {searchResults.map((result) => {
                      ...
                      return (
                        <article className="search-result" key={result.videoId}>
                          <img src={result.thumbnail} alt="" loading="lazy" decoding="async" />
                          <div>
                    ...
                    {externalClipboardCandidate ? (
                      <div className="clipboard-link-card">
                        ...
                        <div className="clipboard-track">
                          <img src={externalClipboardCandidate.thumbnail} alt="" decoding="async" />
                          <div>
              ...
              <div className="room-panel-page">
                ...
                {qrDataUrl && (
                  <div className="qr-block room-qr">
                    <img src={qrDataUrl} alt={`Join ${activeRoomId}`} decoding="async" />
                    <span>
                      <QrCode aria-hidden="true" />
                      Scan to join
                    </span>
  ...
  return (
    ...
    >
      ...
      {qrDataUrl && roomId && (
        ...
        >
          <img src={qrDataUrl} alt="" decoding="async" />
          <span>{roomId}</span>
        </div>
      )}
      {!song?.videoId && (
    ...
    <div className="tutorial-backdrop" role="dialog" aria-modal="true" aria-labelledby="external-search-tutorial-title">
      <section className="tutorial-card">
        ...
        <div className="tutorial-phone" aria-hidden="true">
          ...
          <div className="tutorial-step step-7">7. Tap Add Copied Song</div>
        </div>
...
function AvatarBadge({ avatarId, member, size = "sm" }) {
  ...
  return (
    <span
      ...
      title={avatar.name}
    >
      {avatar.image ? <img src={avatar.image} alt="" loading="lazy" decoding="async" /> : avatar.icon}
    </span>
  );
}
...
function AppIcon() {
  return <img className="app-icon" src={APP_ICON_URL} alt="" aria-hidden="true" decoding="async" />;
}

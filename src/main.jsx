...
function App() {
  ...
  // ── Games: search YouTube for a genre/decade query and return video list ──
  async function searchYouTubeForGame(queryText, maxResults = 20) {
    ...
    >
          ...
          <div className="menu-wrap">
            ...
            {menuOpen && (
              <div className="overflow-menu">
                ...
                <button onClick={() => { setGamesOpen(true); setMenuOpen(false); }} type="button">
      ...
      {gamesOpen && (
        <GamesHub
          ...
          songs={songs}
          roomId={activeRoomId}
          searchYouTubeForGame={searchYouTubeForGame}
          fetchYouTubeLinkDetails={fetchYouTubeLinkDetails}
          playlistTrackDisplay={playlistTrackDisplay}
...
// ═══════════════════════════════════════════════════════════════════
//  ROCK SOLID MUSIC CHALLENGE — integrated into PartyBeats as Games
// ═══════════════════════════════════════════════════════════════════

const GAME_MODES = [
  { id: "name-that-tune",  label: "Name That Tune",     icon: "🎵", desc: "Name the song title" },
  { id: "guess-artist",    label: "Guess the Artist",   icon: "🎤", desc: "Who performs this?" },
  { id: "guess-decade",    label: "Guess the Decade",   icon: "📅", desc: "When was this released?" },
  { id: "finish-lyric",    label: "Finish the Lyric",   icon: "💬", desc: "Complete the line" },
];
const DECADES = ["1950s","1960s","1970s","1980s","1990s","2000s","2010s","2020s"];

const QUICK_SEARCHES = [
  "80s hits", "90s pop", "70s rock", "2000s hip hop", "classic rock",
  "pop hits 2020s", "country hits", "R&B classics", "disco hits", "punk rock",
  "90s alternative", "80s metal", "60s rock", "latin hits", "reggae classics"
];
...
function gameParseTitle(rawTitle, channelName = "") {
  // Strip noise — reuse PartyBeats' proven approach
  const clean = rawTitle
    .replace(/\s*\[[^\]]*\]\s*/g, " ")
    .replace(/\s*\([^)]*(official|video|audio|lyrics?|visualizer|hd|hq|live|remaster)[^)]*\)\s*/gi, " ")
    .replace(/\s*\|\s*.*/g, "")
    .replace(/\s+/g, " ")
    .trim();
  ...
  const cleanChannel = (channelName || "")
    .replace(/\s*-\s*topic\b/gi, "")
    .replace(/vevo$/i, "")
    .replace(/\s*(official|music|tv|channel|records?)$/i, "")
    .trim();

  const separator = clean.match(/\s[-–—]\s/);
  if (separator) {
    const artist = clean.slice(0, separator.index).trim();
    const title  = clean.slice(separator.index + separator[0].length).trim();
    // Use channel name to confirm / clean artist
    return { artist: cleanChannel || artist, title: title || clean };
  }
  return { title: clean, artist: cleanChannel };
}

function gameCalcPoints(timeElapsed, timeLimit, base = 100) {
  const ratio = 1 - (timeElapsed / timeLimit);
  return Math.max(5, Math.round(base * (0.2 + 0.8 * ratio)));
}

// ── YouTube hidden player for game audio ──────────────────────────
function GameYouTubePlayer({ videoId, onReady, playerRef }) {
  ...
  React.useEffect(() => {
    ...
    async function mount() {
      // Wait for YT API (already loaded by PartyBeats)
      let attempts = 0;
      while (!window.YT?.Player && attempts < 40) {
      ...
      div.id = "game-yt-" + Date.now();
      containerRef.current.innerHTML = "";
      containerRef.current.appendChild(div);

      instanceRef.current = new window.YT.Player(div.id, {
    ...
    return () => {
      cancelled = true;
      try { instanceRef.current?.stopVideo(); } catch {}
    };
  }, [videoId]);
...
function GamePlaylistBuilder({ existingRoomSongs, playlistTrackDisplay, fetchYouTubeLinkDetails,
  searchYouTubeForGame, YOUTUBE_API_KEY, onPlaylistReady, onBack }) {
  ...
  // Pre-populate from room playlist
  React.useEffect(() => {
    if (existingRoomSongs && existingRoomSongs.length > 0 && gamePlaylist.length === 0) {
      const mapped = existingRoomSongs.slice(0, 30).map(s => {
        const display = playlistTrackDisplay(s);
        return {
          id: gameGenId(),
          videoId: s.videoId,
          title: display.title || s.title || "Unknown",
          artist: display.artist || s.artist || "Unknown",
          source: "room"
        };
      });
      setGamePlaylist(mapped);
    }
  }, []);
  ...
  async function handleAutoSearch(q) {
    const query = (q || searchQuery).trim();
    if (!query) return;
    if (!YOUTUBE_API_KEY) { showToast("Add VITE_YOUTUBE_API_KEY to search YouTube automatically"); return; }
    setAutoLoading(true);
    const results = await searchYouTubeForGame(query + " official audio", 20);
    setAutoLoading(false);
    if (!results.length) { showToast("No results found"); return; }
    setAutoResults(results);
  }

  function addAutoResult(r) {
    if (gamePlaylist.some(s => s.videoId === r.videoId)) {
      showToast("Already in playlist");
      return;
    }
    const parsed = gameParseTitle(r.title, r.channelTitle);
    setGamePlaylist(p => [...p, {
      id: gameGenId(),
      videoId: r.videoId,
      title: parsed.title || r.title,
      artist: parsed.artist || r.channelTitle,
      source: "youtube"
    }]);
    showToast(`✓ Added: ${parsed.title || r.title}`);
  }

  function addAllResults() {
    const newSongs = autoResults
      .filter(r => !gamePlaylist.some(s => s.videoId === r.videoId))
      .map(r => {
        const parsed = gameParseTitle(r.title, r.channelTitle);
        return { id: gameGenId(), videoId: r.videoId, title: parsed.title || r.title, artist: parsed.artist || r.channelTitle, source: "youtube" };
      });
    setGamePlaylist(p => [...p, ...newSongs]);
    showToast(`✓ Added ${newSongs.length} songs`);
  }
  ...
  async function handleClipboard() {
    ...
    try {
      ...
      const videoId = cleanYouTubeVideoId(extractYouTubeVideoId(text));
      if (!videoId) { showToast("No YouTube link found in clipboard"); return; }
      if (gamePlaylist.some(s => s.videoId === videoId)) { showToast("Already in playlist"); return; }
      const details = await fetchYouTubeLinkDetails(videoId);
      const parsed = gameParseTitle(details.title, details.channelTitle);
      setGamePlaylist(p => [...p, {
        id: gameGenId(),
        videoId,
        title: parsed.title || details.title,
        artist: parsed.artist || details.channelTitle,
        source: "clipboard"
      }]);
      try { await navigator.clipboard.writeText(""); } catch {}
      showToast(`✓ Added: ${parsed.title || details.title}`);
    } catch {
      showToast("Clipboard access denied — copy a YouTube link first");
    } finally {
  ...
  function removeTrack(id) {
    setGamePlaylist(p => p.filter(s => s.id !== id));
  }
  ...
  return (
    <div className="games-panel">
      ...
      <div className="games-panel-header">
        <button className="games-back-btn" onClick={onBack} type="button">← Back</button>
        <h3>Build Game Playlist</h3>
        <span className="games-count">{gamePlaylist.length} songs</span>
      </div>

      <div className="games-playlist-builder">
        {/* Auto-search by genre/decade */}
        <div className="games-section">
          <label className="games-section-label">🔍 Search by Genre or Decade</label>
          <div className="games-search-row">
            <input
              ...
              placeholder="e.g. 80s hits, 90s pop, classic rock..."
            />
            <button className="games-primary-btn" onClick={() => handleAutoSearch()} disabled={autoLoading} type="button">
              {autoLoading ? "..." : <><Search size={14}/> Search</>}
            </button>
          </div>
          <div className="games-quick-searches">
            {QUICK_SEARCHES.map(q => (
              <button key={q} className="games-quick-chip" onClick={() => { setSearchQuery(q); handleAutoSearch(q); }} type="button">{q}</button>
            ))}
          </div>
        </div>
        ...
        {/* Clipboard add */}
        <div className="games-section">
          <label className="games-section-label">📋 Add from Clipboard</label>
          <button className="games-primary-btn games-clipboard-btn" onClick={handleClipboard} disabled={clipboardLoading} type="button">
            {clipboardLoading ? "Checking..." : "📋 Add from Clipboard"}
          </button>
          <span className="games-muted">Copy a YouTube or YouTube Music link, then tap above</span>
        </div>

        {/* Auto-search results */}
        {autoResults.length > 0 && (
          <div className="games-section">
            <div className="games-section-label-row">
              <label className="games-section-label">Search Results</label>
              <button className="games-link-btn" onClick={addAllResults} type="button">Add All ({autoResults.length})</button>
            </div>
            <div className="games-auto-results">
              {autoResults.map(r => {
                const parsed = gameParseTitle(r.title, r.channelTitle);
                const inPlaylist = gamePlaylist.some(s => s.videoId === r.videoId);
                return (
                  <div key={r.videoId} className={`games-result-row ${inPlaylist ? "is-added" : ""}`}>
                    <img src={r.thumbnail} alt="" className="games-thumb" />
                    <div className="games-result-info">
                      <strong>{parsed.title || r.title}</strong>
                      <span>{parsed.artist || r.channelTitle}</span>
                    </div>
                    <button className="games-add-btn" onClick={() => addAutoResult(r)} disabled={inPlaylist} type="button">
                      {inPlaylist ? <CheckCircle2 size={16}/> : <Plus size={16}/>}
                    </button>
                  </div>
                );
              })}
        ...
        <div className="games-section">
          <div className="games-section-label-row">
            <label className="games-section-label"><ListMusic size={13}/> Playlist</label>
            {gamePlaylist.length > 0 && (
              <button className="games-link-btn games-danger-link" onClick={() => setGamePlaylist([])} type="button">Clear all</button>
            )}
          </div>
          {gamePlaylist.length === 0 ? (
            <div className="games-empty">No songs yet — search above or add from clipboard</div>
          ) : (
            <div className="games-track-list">
              {gamePlaylist.map((s, i) => (
                <div key={s.id} className="games-track-row">
                  <span className="games-track-num">{i + 1}</span>
                  <div className="games-track-info">
                    <strong>{s.title}</strong>
                    <span>{s.artist}</span>
                  </div>
                  <span className={`games-source-badge games-source-${s.source}`}>
                    {s.source === "room" ? "room" : s.source === "clipboard" ? "clip" : "yt"}
                  </span>
                  <button className="games-remove-btn" onClick={() => removeTrack(s.id)} type="button"><X size={13}/></button>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="games-footer">
        <button
          className="games-primary-btn games-full-btn"
          onClick={() => gamePlaylist.length >= 2 && onPlaylistReady(gamePlaylist)}
          disabled={gamePlaylist.length < 2}
          type="button"
        >
          <Gamepad2 size={16}/> Start Game ({gamePlaylist.length} songs) →
        </button>
      </div>
    </div>
  );
...
// ── Game Setup panel ───────────────────────────────────────────────
function GameSetup({ playlist, onStart, onBack }) {
  const [modes, setModes] = useState(["name-that-tune", "guess-artist"]);
  const [players, setPlayers] = useState([
    { id: gameGenId(), name: "Player 1", score: 0, avatar: PLAYER_AVATARS[0] },
    { id: gameGenId(), name: "Player 2", score: 0, avatar: PLAYER_AVATARS[1] }
  ]);
  const [roundCount, setRoundCount] = useState(Math.min(10, playlist.length));
  ...
  function addPlayer() {
    const name = playerName.trim() || `Player ${players.length + 1}`;
    setPlayers(p => [...p, { id: gameGenId(), name, score: 0, avatar: PLAYER_AVATARS[p.length % PLAYER_AVATARS.length] }]);
    setPlayerName("");
  }
  ...
  return (
    <div className="games-panel">
      <div className="games-panel-header">
        <button className="games-back-btn" onClick={onBack} type="button">← Back</button>
        <h3>Game Setup</h3>
        <span className="games-count">{playlist.length} songs</span>
      </div>

      <div className="games-setup-body">
        <div className="games-section">
          <label className="games-section-label">Game Modes</label>
          <div className="games-modes-grid">
            {GAME_MODES.map(m => (
              <button
                key={m.id}
                className={`games-mode-card ${modes.includes(m.id) ? "is-selected" : ""}`}
                onClick={() => toggleMode(m.id)}
                type="button"
              >
                <span className="games-mode-icon">{m.icon}</span>
        ...
        <div className="games-section">
          <label className="games-section-label">Players</label>
          <div className="games-player-list">
            {players.map((p, i) => (
              <div key={p.id} className="games-player-chip">
                <span>{p.avatar}</span>
                <span>{p.name}</span>
                {players.length > 2 && (
                  <button className="games-remove-btn" onClick={() => setPlayers(pl => pl.filter(x => x.id !== p.id))} type="button"><X size={11}/></button>
                )}
              </div>
            ))}
          </div>
          <div className="games-add-player-row">
            <input className="games-input" value={playerName} onChange={e => setPlayerName(e.target.value)}
              onKeyDown={e => e.key === "Enter" && addPlayer()} placeholder="Add player name..." />
            <button className="games-primary-btn" onClick={addPlayer} type="button"><Plus size={14}/></button>
          </div>
        </div>

        <div className="games-section">
          <label className="games-section-label">Rounds: <strong>{roundCount}</strong></label>
          <input type="range" min={2} max={Math.min(30, playlist.length)} value={roundCount}
            onChange={e => setRoundCount(Number(e.target.value))} className="games-range" />
        </div>

        <div className="games-section">
          <label className="games-section-label">Play time per song: <strong>{playSeconds}s</strong></label>
          <input type="range" min={5} max={30} step={5} value={playSeconds}
            onChange={e => setPlaySeconds(Number(e.target.value))} className="games-range" />
          <div className="games-range-labels">
      ...
      <div className="games-footer">
        <button className="games-primary-btn games-full-btn" onClick={() => onStart({ modes, players: players.map(p=>({...p,score:0,streak:0})), roundCount, playSeconds })} type="button">
          <Zap size={16}/> Let's Play!
        </button>
      </div>
    </div>
  );
...
// ── Active game screen ─────────────────────────────────────────────
function GamePlay({ playlist, config, onEnd }) {
  const { modes, players: initPlayers, roundCount, playSeconds } = config;
  const [players, setPlayers] = useState(initPlayers);
  const [roundIdx, setRoundIdx] = useState(0);
  const [phase, setPhase] = useState("countdown"); // countdown | playing | buzzed | result | done
  const [countdown, setCountdown] = useState(3);
  const [timeLeft, setTimeLeft] = useState(playSeconds);
  const [buzzedId, setBuzzedId] = useState(null);
  ...
  const song = playlist[roundIdx % playlist.length];
  const mode = modes[roundIdx % modes.length];
  const modeInfo = GAME_MODES.find(m => m.id === mode);
  const timePercent = (timeLeft / playSeconds) * 100;
  ...
  React.useEffect(() => {
    ...
    const t = setInterval(() => {
      n--;
      if (n <= 0) { clearInterval(t); setPhase("playing"); setTimeLeft(playSeconds); }
      else setCountdown(n);
    }, 900);
    return () => clearInterval(t);
  }, [roundIdx, phase === "countdown"]);
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft(t => {
        if (t <= 1) {
          clearInterval(timerRef.current);
          stopAudio();
          setLastResult({ song, winnerId: null, points: 0 });
          setPhase("result");
          return 0;
        }
        return t - 1;
  ...
  function handleBuzz(playerId) {
    ...
    clearInterval(timerRef.current);
    stopAudio();
    setBuzzedId(playerId);
    setPhase("buzzed");
    setAnswerText("");
  }

  function submitAnswer(answer) {
    const correct = mode === "guess-decade"
      ? answer === song.artist // decades stored differently; just accept any tap
      : answer.toLowerCase().trim().includes(
          (mode === "guess-artist" ? song.artist : song.title).toLowerCase().trim().slice(0, 4)
        );

    const pts = correct ? gameCalcPoints(playSeconds - timeLeft, playSeconds) : 0;
    setPlayers(ps => ps.map(p => p.id === buzzedId
      ? { ...p, score: p.score + pts, streak: correct ? (p.streak||0)+1 : 0 }
      : p
    ));
    setLastResult({ song, winnerId: correct ? buzzedId : null, points: pts, correct });
    setPhase("result");
  }

  function hostAward(playerId) {
    const pts = gameCalcPoints(playSeconds - timeLeft, playSeconds);
    setPlayers(ps => ps.map(p => p.id === playerId ? { ...p, score: p.score + pts } : p));
    setLastResult({ song, winnerId: playerId, points: pts, correct: true });
    setPhase("result");
  }

  function hostWrong() {
    setLastResult({ song, winnerId: null, points: 0, correct: false });
    setPhase("result");
  }
  ...
  function nextRound() {
    ...
    if (next >= roundCount) { setPhase("done"); return; }
    setRoundIdx(next);
    setBuzzedId(null);
    setPhase("countdown");
  }
  ...
  if (phase === "done") {
    ...
    return (
      <div className="games-panel games-gameover">
        <div className="games-gameover-trophy">🏆</div>
        <h2 className="games-gameover-title">{sorted[0]?.avatar} {sorted[0]?.name} Wins!</h2>
        <div className="games-final-scores">
          {sorted.map((p, i) => (
            <div key={p.id} className={`games-score-row rank-${i+1}`}>
              <span className="games-rank">{["🥇","🥈","🥉"][i] || `${i+1}`}</span>
              <span className="games-score-avatar">{p.avatar}</span>
              <span className="games-score-name">{p.name}</span>
              <span className="games-score-pts">{p.score}</span>
            </div>
          ))}
        </div>
        <button className="games-primary-btn games-full-btn" onClick={onEnd} type="button">← Back to Games</button>
      </div>
    );
  }

  return (
    <div className="games-panel games-active">
      {/* Hidden YT player */}
      {song.videoId && phase !== "countdown" && (
        <GameYouTubePlayer
      ...
      {phase === "countdown" && (
        <div className="games-countdown-overlay">
          <div className="games-countdown-num" key={countdown}>{countdown || "GO!"}</div>
          <div className="games-countdown-song">{modeInfo?.icon} {modeInfo?.label}</div>
        </div>
      )}

      {/* HUD */}
      <div className="games-hud">
        <span className="games-hud-round">Round {roundIdx+1}/{roundCount}</span>
        <div className="games-hud-center">
          <span className={`games-timer ${isUrgent ? "is-urgent" : ""}`}>{timeLeft}s</span>
          {song.videoId && <span className="games-audio-status">{audioReady ? "♪" : "⏳"}</span>}
        </div>
        <span className="games-mode-badge">{modeInfo?.icon} {modeInfo?.label}</span>
      </div>

      {/* Progress bar */}
      <div className="games-progress-track">
        <div className={`games-progress-fill ${isUrgent ? "is-urgent" : ""}`} style={{ width: `${timePercent}%` }}/>
      </div>

      {/* Stage */}
      <div className="games-stage">
        <div className={`games-vinyl ${phase === "playing" ? "is-spinning" : ""}`}>🎵</div>
        <p className="games-mode-prompt">{modeInfo?.desc}...</p>
      </div>

      {/* Buzzed-in banner */}
      {phase === "buzzed" && buzzedPlayer && (
        <div className="games-buzzed-banner">
          <span className="games-buzzed-label">Buzzed in!</span>
          <strong>{buzzedPlayer.avatar} {buzzedPlayer.name}</strong>
        </div>
      )}

      {/* Answer input when buzzed */}
      {phase === "buzzed" && buzzedPlayer && (
        <div className="games-answer-zone">
          {mode === "guess-decade" ? (
            <div className="games-decade-grid">
              {DECADES.map(d => (
                <button key={d} className="games-decade-btn" onClick={() => submitAnswer(d)} type="button">{d}</button>
              ))}
            </div>
          ) : (
            <>
              <input
                className="games-answer-input"
                value={answerText}
                onChange={e => setAnswerText(e.target.value)}
                onKeyDown={e => e.key === "Enter" && answerText.trim() && submitAnswer(answerText)}
                placeholder="Type answer..."
                autoFocus
              />
              <button className="games-primary-btn games-full-btn" onClick={() => submitAnswer(answerText)} disabled={!answerText.trim()} type="button">
                Submit Answer
              </button>
            </>
          )}
      ...
      {/* Buzz buttons — one per player when playing */}
      {(phase === "playing" || phase === "waiting") && (
        <div className="games-buzzer-zone">
          {players.map(p => (
            <button key={p.id} className="games-buzz-btn" onClick={() => handleBuzz(p.id)} type="button">
              🟥 {p.avatar} {p.name}
            </button>
          ))}
        </div>
      ...
      {phase === "result" && lastResult && (
        <div className="games-result-overlay">
          <div className="games-result-icon">{lastResult.correct ? "🎉" : "⏱"}</div>
          {lastResult.winnerId ? (
            <>
              <strong className="games-result-winner">
                {players.find(p => p.id === lastResult.winnerId)?.avatar} {players.find(p => p.id === lastResult.winnerId)?.name}
              </strong>
              <div className="games-result-points">+{lastResult.points}</div>
            </>
          ) : (
            <strong className="games-result-winner games-result-nobody">No one buzzed in</strong>
          )}
          <div className="games-reveal-card">
            <span className="games-reveal-label">The answer was</span>
            <strong className="games-reveal-title">{lastResult.song.title}</strong>
            <span className="games-reveal-artist">{lastResult.song.artist}</span>
          </div>
          {/* Host override buttons */}
          {phase === "result" && !lastResult.winnerId && buzzedPlayer && (
            <div className="games-host-override">
              <button className="games-correct-btn" onClick={() => hostAward(buzzedId)} type="button"><CheckCircle2 size={14}/> Correct</button>
              <button className="games-wrong-btn" onClick={hostWrong} type="button"><XCircle size={14}/> Wrong</button>
            </div>
          )}
          <button className="games-primary-btn games-full-btn" onClick={nextRound} type="button">
            {roundIdx + 1 >= roundCount ? "See Final Scores →" : "Next Round →"}
          </button>
        </div>
      )}

      {/* Mini scoreboard */}
      <div className="games-mini-scores">
        {[...players].sort((a,b) => b.score - a.score).map(p => (
          <div key={p.id} className={`games-mini-player ${p.id === buzzedId ? "is-buzzed" : ""}`}>
            <span>{p.avatar}</span>
            <span className="games-mini-name">{p.name}</span>
            <span className="games-mini-score">{p.score}</span>
          </div>
        ))}
      </div>
    </div>
...
// ── Main GamesHub shell ────────────────────────────────────────────
function GamesHub({ onClose, songs, roomId, searchYouTubeForGame, fetchYouTubeLinkDetails, playlistTrackDisplay, YOUTUBE_API_KEY, isDarkTheme }) {
  const [view, setView] = useState("home"); // home | playlist | setup | play
  const [gamePlaylist, setGamePlaylist] = useState([]);
  const [gameConfig, setGameConfig] = useState(null);

  return (
    <div className="games-backdrop" role="dialog" aria-modal="true" aria-label="Rock Solid Music Challenge">
      <div className="games-shell">
        {/* Close button always visible */}
        <button className="games-close-btn" onClick={onClose} type="button" aria-label="Close games">
          <X size={18}/>
        </button>

        {view === "home" && (
          <div className="games-home">
            <div className="games-home-logo">
              <span className="games-home-icon">🎵</span>
              <h2>Rock Solid<br/><span>Music Challenge</span></h2>
              <p>The ultimate multiplayer music party game</p>
            </div>
            <div className="games-home-actions">
              <button className="games-primary-btn games-full-btn games-hero-btn" onClick={() => setView("playlist")} type="button">
                <Gamepad2 size={18}/> Start New Game
              </button>
              {songs.length > 0 && (
                <button className="games-secondary-btn games-full-btn" onClick={() => {
                  const mapped = songs.slice(0, 30).map(s => {
                    const d = playlistTrackDisplay(s);
                    return { id: gameGenId(), videoId: s.videoId, title: d.title || s.title, artist: d.artist || s.artist, source: "room" };
                  });
                  setGamePlaylist(mapped);
                  setView("setup");
                }} type="button">
                  <ListMusic size={16}/> Use Room Playlist ({Math.min(30, songs.length)} songs)
                </button>
              )}
            </div>
            <div className="games-home-modes">
              {GAME_MODES.map(m => (
                <div key={m.id} className="games-home-mode-chip">
                  {m.icon} {m.label}
                </div>
              ))}
            </div>
        ...
        {view === "playlist" && (
          <GamePlaylistBuilder
            existingRoomSongs={songs}
            playlistTrackDisplay={playlistTrackDisplay}
            fetchYouTubeLinkDetails={fetchYouTubeLinkDetails}
            searchYouTubeForGame={searchYouTubeForGame}
            YOUTUBE_API_KEY={YOUTUBE_API_KEY}
            onPlaylistReady={pl => { setGamePlaylist(pl); setView("setup"); }}
            onBack={() => setView("home")}
          />
        )}
        ...
        {view === "setup" && (
          <GameSetup
            ...
            onStart={cfg => { setGameConfig(cfg); setView("play"); }}
            onBack={() => setView("playlist")}
          />
        )}

        {view === "play" && gameConfig && (
          <GamePlay
            playlist={gamePlaylist}
            config={gameConfig}
            onEnd={() => { setView("home"); setGameConfig(null); }}
          />
        )}
      </div>
    </div>

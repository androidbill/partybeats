...
const TOGGLES = [
  ...
  { key: "partyMotionEnabled", label: "Party motion", note: "Ambient stage lights and embers behind the room" }
];
...
export function ThemePicker({ currentTheme, onPick, onClose, motion }) {
  return (
    <Modal title="Room look" subtitle="Only changes this device." onClose={onClose}>
      {motion && (
        <div className="motion-control">
          <p className="motion-control-label">Party motion on this device</p>
          <div className="segmented" role="radiogroup" aria-label="Party motion">
            {[["", "Follow room"], ["on", "On"], ["off", "Off"]].map(([value, label]) => (
              <button
                key={label}
                className={motion.value === value ? "segment is-active" : "segment"}
                onClick={() => motion.onChange(value)}
                role="radio"
                aria-checked={motion.value === value}
                type="button"
              >
                {label}
              </button>

...
import {
  DoorOpen, Megaphone, Palette, Plus, Settings, Share2, SmilePlus, Sparkles, Users
} from "lucide-react";
...
import ClipboardCatch from "./ClipboardCatch";
import PartyMotion from "./PartyMotion";
import { MomentsPanel, useMoments } from "./Moments";
import {
...
export default function Room({ auth, session, deviceId, now, showToast, onOpenThemes, motionOverride }) {
  const { user } = auth;
  const {
  ...
  const [momentsSeenAt, setMomentsSeenAt] = useState(() => Date.now());
  ...
  const moments = useMoments({ songs, messages, shouts, reactions, members });
  const unseenMoments = useMemo(
    () => moments.filter((item) => item.at > momentsSeenAt).length,
    [moments, momentsSeenAt]
  );
  const partyMotionOn = motionOverride === "on"
    || (motionOverride !== "off" && room?.partyMotionEnabled !== false);
  ...
  return (
    <div className="room">
      {partyMotionOn && <PartyMotion />}
      <FloatingReactions items={floatingItems} />
      <ShoutBanner shout={activeShout} />
      <ClipboardCatch isAdmin={isAdmin} onAdd={addTrack} showToast={showToast} />
      ...
      <header className="room-header">
        ...
        <div className="room-header-actions">
          <button
            className="icon-button moments-button"
            onClick={() => { setMomentsSeenAt(Date.now()); setOpenPanel("moments"); }}
            title="Moments"
            type="button"
          >
            <Sparkles aria-hidden="true" />
            {unseenMoments > 0 && <span className="moments-badge">{unseenMoments > 9 ? "9+" : unseenMoments}</span>}
          </button>
      ...
      {openPanel === "moments" && (
        <MomentsPanel items={moments} now={now} onClose={() => { setMomentsSeenAt(Date.now()); setOpenPanel(""); }} />
      )}
      {openPanel === "share" && (

...
BP PartyBeats is a Firebase-backed PWA for party music rooms. Google-authenticated users can create or join rooms. Anonymous nickname users can join rooms only. Room admins control the YouTube player and queue; non-admins can add YouTube songs every 3 minutes; everyone can react to queued songs with emojis.
...
1. Go to [Firebase Console](https://console.firebase.google.com/) and create a project named `BP PartyBeats`.
...
Rooms stay available until an admin closes them or you manually delete them from
Firestore. The `lastActivityAt` field is updated as rooms are used, so you can
sort old rooms by that timestamp when cleaning them up.
...
You can paste YouTube URLs without any extra setup. To search YouTube inside BP PartyBeats:

1. Open [Google Cloud Console](https://console.cloud.google.com/).
2. Select the same project or create one for BP PartyBeats.
